...
LightingResult DoPointLight(float3 dirToCamera, float3 vertPos, float3 norm, float specPow)
{
    LightingResult res;

    float3 dirToLight = Light.Position.xyz - vertPos;
    ...
    res.Diffuse = CalculateDiffuse(dirToLight, norm) * att;
    res.Specular = CalculateSpecular(dirToCamera, dirToLight, norm, specPow) * att;
...
float4 PSMain(PS_IN input) : SV_Target
{
    ...
    LightingResult lightRes = DoPointLight(dirToCamera, gBuf.WorldPos, gBuf.Normal, gBuf.Specular.a);
